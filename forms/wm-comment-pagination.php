...
if( !function_exists( 'wm_comment_pagination' ) ){
    ...
    function wm_comment_pagination(){
        ...
        if( $options && count($options) == 4 ){
            ...
            // 3 => Page number.
            $pagination = $WM_posts->get_comment_pagination( $options[0], $options[2], $options[1], $options[3] );

            $comments = $WM_posts->get_comments( $options[0], $options[2], $options[1], $options[3] );
